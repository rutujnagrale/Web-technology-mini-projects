...
$sql = "USE book_store";
$conn->query($sql);
$sql = "SELECT customer_id from customer WHERE user_id = ".$_SESSION['id']."";
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
$cID = $row['customer_id'];
}
$sql = "UPDATE cart SET customer_id = ".$cID." WHERE 1";
$conn->query($sql);
$sql = "SELECT * FROM cart";
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
$sql = "INSERT INTO `order`(customer_id, book_id, date_purchase, quantity, total_price, status) 
VALUES(".$row['customer_id'].", '".$row['book_id']
."', CURRENT_TIME, ".$row['quantity'].", ".$row['total_price'].", 'N')";
$conn->query($sql);
}
$sql = "DELETE FROM cart";
$conn->query($sql);
$sql = "SELECT customer.customer_name, customer.customer_ic, customer.customer_gender, customer.customer_address, 
customer.customer_email, customer.customer_phone, book.book_title, book.price, book.image, `order`.`date_purchase`, 
`order`.`quantity`, `order`.`total_price`
FROM customer, book, `order`
WHERE `order`.`customer_id` = customer.customer_id AND `order`.`book_id` = book.book_id AND `order`.`status` = 'N' AND
...
echo "<tr><td>No.Number: </td><td>".$row['customer_ic']."</td></tr>";
echo "<tr><td>E-mail: </td><td>".$row['customer_email']."</td></tr>";
echo "<tr><td>Mobile Number: </td><td>".$row['customer_phone']."</td></tr>";
echo "<tr><td>Gender: </td><td>".$row['customer_gender']."</td></tr>";
echo "<tr><td>Address: </td><td>".$row['customer_address']."</td></tr>";
echo "<tr><td>Date: </td><td>".$row['date_purchase']."</td></tr>";
echo "</blockquote>";
$sql = "SELECT customer.customer_name, customer.customer_ic, customer.customer_gender, customer.customer_address, 
customer.customer_email, customer.customer_phone, book.book_title, book.price, book.image, `order`.`date_purchase`, 
`order`.`quantity`, `order`.`total_price`
...
echo "<tr><td style='background-color: #ccc;'></td><td style='text-align: right;background-color: #ccc;''>Total Price: 
<b>RM".$total."</b></td></tr>";
echo "</table>";
echo "</div>";
$sql = "UPDATE `order` SET status = 'y' WHERE customer_id = ".$cID."";
$conn->query($sql);
}
...
$sql = "USE book_store";
$conn->query($sql);
$sql = "INSERT INTO customer(customer_name, customer_phone, customer_ic, customer_email, 
customer_address, customer_gender) 
VALUES('".$name."', '".$contact."', '".$ic."', '".$email."', '".$address."', 
'".$gender."')";
$conn->query($sql);
$sql = "SELECT customer_id from customer WHERE customer_name = '".$name."' AND 
customer_ic = '".$ic."'";
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
$cID = $row['customer_id'];
...
$result = $conn->query($sql);
while($row = $result->fetch_assoc()){
$sql = "INSERT INTO `order`(customer_id, book_id, date_purchase, quantity, 
total_price, status) 
VALUES(".$row['customer_id'].", '".$row['book_id']
."', CURRENT_TIME, ".$row['quantity'].", ".$row['total_price'].", 'N')";
$conn->query($sql);
